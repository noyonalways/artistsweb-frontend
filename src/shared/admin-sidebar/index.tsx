        ...
        <nav className="px-4 py-4">
          {navItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="flex items-center px-2 py-2 mt-2 text-base font-medium text-gray-600 rounded-md hover:bg-gray-100 hover:text-gray-900 group transition-colors duration-200"
              onClick={() => setOpen(false)}
            >
              <item.icon className="w-6 h-6 mr-4 text-gray-500 group-hover:text-gray-900" />
              {item.name}
            </Link>
          ))}
        </nav>
